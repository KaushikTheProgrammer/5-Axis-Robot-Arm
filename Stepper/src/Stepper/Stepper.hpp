...
class Stepper {
	...
	public:
        Stepper();
		Stepper(const int DIRECTION_PIN, const int PULSE_PIN, const int MICRO_STEP_SIZE, float PROP_ACCEL);
	...
	private:
		...
		float _propAccel = 0.2;   		        			// Proportion of stepper routine to be accelerating/decelerating
		...
		int _frqcy = 100;               						// Frequency of timer in ticks per second (100 for RPI 3)
